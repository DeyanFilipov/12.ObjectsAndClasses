package ObjectsAndClasses.MoreExercises.TeamworkProjects_04;
...
public class Main {
    public static void main(String[] args) {
        ...
        while (!"end of assignment".equals(input = scanner.nextLine())) {
            ...
            } else if ((teamsList.stream().flatMap(team -> team.getMembers().stream()).anyMatch(member -> member.equals(user)))) {
        ...
        teamsList.stream().filter(current -> current.getMembers().isEmpty()).forEach(currentTeam -> System.out.printf("%s\n", currentTeam.getName()));
    }
}
